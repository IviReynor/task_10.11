...
const filterFruits = () => {
  ...
  return fruits.filter((item) => {
    if (parseInt(maxWeightInput.value) < parseInt(minWeightInput.value)) {
      [maxWeightInput.value, minWeightInput.value] = [
        minWeightInput.value,
        maxWeightInput.value,
      ];
    }
    return (
      item.weight >= parseInt(minWeightInput.value) &&
      item.weight <= parseInt(maxWeightInput.value)
    );
  });
};
...
const sortAPI = {
  ...
  quickSort(arr, comparation) {
    ...
    return [
      ...this.quickSort(less, comparation),
      currentItem,
      ...this.quickSort(more, comparation),
    ];
  },
